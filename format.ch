...
if (!w_open_in(&fmt_file)) return 1;
if (!load_fmt_file()) {
  w_close(&fmt_file);
  exit(0);
} 
w_close(&fmt_file);
#endif
@z above code was taken verbatim from @<Get the first line...@> (except that open_fmt_file is replaced with w_open_in and final newline is not produced because `return' is not changed by macro)
