...
is used for writing text files in all cases but one.
...
its old expansion.
\medskip
In addition to redefining the macro {\it pascal\_write\/}, we need to add the
`|L|' prefix to strings
which are used in the macros that call the macro {\it pascal\_write\/}.
These changes are trivial and there are quite a few of
them so we will not list them here. Instead, we show the following
cases, where the conversion specifier
in the {\it printf}-style directives also needs to change:
\verbatim
< wterm("%c",xchr[s]);
> wterm(L"%lc",xchr[s]);
...
\endverbatim
\begingroup
\everyverbatim={\advance\parindent by -28pt }
\verbatim
< pascal_write(write_file[selector],"%c",xchr[s]);
> pascal_write(write_file[selector],L"%lc",xchr[s]);
\endverbatim
\endgroup
