...
\def\thetitle{\UTF-8 installations of \TeX}% ugly workaround
...
to |fwprintf| in the definition of {\it write\/}.
The one case where {\it write\/} is used is
for writing \DVI\ files \Dash there we just use
its old expansion.
\medskip
In addition to redefining the macro {\it write\/}, we need to add the
`|L|' prefix to strings
which are used in the macros that call the macro {\it write\/}.
...
\endverbatim
\begingroup
\everyverbatim={\advance\parindent by -6.5pt }
\verbatim
