all:
	...
	tie -c tex.ch tex.w constants.ch 64bit.ch special.ch paper+origin.ch $(CHF) >/dev/null
	ctangle tex tex
...
CHF=charset.ch path.ch interrupt.ch arg.ch print.ch preload.ch time.ch log.ch edit.ch name.ch exit.ch input.ch close.ch
