  ...
  if (base_ptr > 0) {
    ...
    if (interaction > batch_mode) { /* FIXME: why we check |interaction|?
      ...
      for (pool_pointer j=str_start[input_stack[base_ptr].name_field];
        ...
        for (int i = 0; i < len; i++) {
          incr(k);
          if (k <= sizeof ed_name0 - 1) ed_name0[k-1] = mb[i];
          else if (k - i <= sizeof ed_name0 - 1)
            for (int x = k - i; x <= sizeof ed_name0 - 1; x++)
              ed_name0[x-1] = '\0';
        }
      }
      if (k <= sizeof ed_name0 - 1) ed_name0[k]='\0';
      else ed_name0[sizeof ed_name0-1] = '\0';
      char cmd[500];
      if (snprintf(cmd, sizeof cmd,
           (strcmp("TeXinputs/", ed_name0) == 0 ? "em /home/user/ctex/%s %d" : "em %s %d"),
           ed_name0, line) >= sizeof cmd)
