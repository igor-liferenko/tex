...
\medskip
In |tex.web| the type |char| is able to contain the same
characters as in text files (see \S19 in [3]).
Text characters are now in Unicode.
\verbatim
...
\endverbatim
File names are in \UTF-8 (on most systems). Thus,
|pascal_char| is changed to |char|,
not to |wchar_t|. This is does not contradict to |tex.web|
(see comment to |name_of_file| in \S26 in [3]).
\verbatim
